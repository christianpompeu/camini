...
### E.3 - Observabilidade do Pipeline RAG e Disciplina de Saída
- **Tracking de Expansão de Contexto:** O `schema-engine.ts` foi refatorado para expor metadados da composição de contexto. Agora o log diferencia perfeitamente as `RouterRawTables` (devolvidas pelo Roteador Semântico), as `SeedTables` (Roteador + Fallback Léxico) e as `ExpandedTables` (adicionadas deterministicamente via Grafo, como a inferência de `TMOVPAGTO` ao ler "pagamento").
- **Observabilidade Holística (Request Total):** O orquestrador em `route.ts` agora utiliza `performance.now()` para capturar a `RequestPipelineLatency` real. Os logs `[RESULT]` foram atualizados para consolidar tokens e tempos gastos no pipeline _inteiro_ (Router + Generator + eventuais Repairs) ao invés de exibir erroneamente apenas o consumo parcial do gerador, provendo métricas confiáveis para testes A/B futuros.
